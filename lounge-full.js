...
if (!confirm(message)) { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); return false; }
}, true);
})();
...
notifySound.preload = "auto"; notifySound.volume = VOLUME;
notifySound.setAttribute("data-lounge-custom-alert","1");
function playCustomSound() {
var now = Date.now();
if (now - lastPlay < MIN_GAP_MS) return;
lastPlay = now;
try { notifySound.pause(); notifySound.currentTime=0; notifySound.play().catch(function(){}); } catch(e){}
}
...
playCustomSound(); return Promise.resolve();
}
} catch(e){}
return originalPlay.apply(this, arguments);
};
}
function isNotifyContainer(el) { return el && el.nodeType===1 && el.classList && el.classList.contains("hand") && el.classList.contains("corner") && el.classList.contains("nosel"); }
function isTargetAlertBox(el) { return (isNotifyContainer(el) && el.querySelector && el.querySelector(".NotReply")) || (isNotifyContainer(el) && el.textContent && el.textContent.indexOf("حصلت على إعجاب")!==-1); }
function scanAlertNode(node) {
if (!node || node.nodeType!==1) return;
if (isTargetAlertBox(node)) { playCustomSound(); return; }
if (node.querySelectorAll) { var boxes=node.querySelectorAll(".hand.corner.nosel"); for(var i=0;i<boxes.length;i++) if(isTargetAlertBox(boxes[i])){ playCustomSound(); return; } }
}
function getChatKey(chatEl) {
if (!chatEl) return "";
var k=chatEl.getAttribute("data-lid"); if(k) return k;
var h=chatEl.querySelector(".uhashe,.thUserHashExtra"); if(h&&h.textContent){k=h.textContent.replace(/\s+/g,"").trim();if(k)return k;}
if(chatEl.id) return chatEl.id;
var o=chatEl.querySelector("[onclick*='openw']"); if(o){var m=(o.getAttribute("onclick")||"").match(/openw\(['"]([^'"]+)['"]/);if(m)return m[1];}
return "";
}
function findChatWrapper(el) { if(!el||el.nodeType!==1) return null; if(el.classList&&el.classList.contains("cc")) return el; return el.closest?el.closest(".cc"):null; }
function scanPrivateChats() {
var chats=document.querySelectorAll(".cc"),cur=Object.create(null),play=false;
for(var i=0;i<chats.length;i++){var k=getChatKey(chats[i]);if(!k)continue;if(chats[i].classList&&chats[i].classList.contains("unread")){cur[k]=true;if(!soundedUnreadChats[k]){soundedUnreadChats[k]=true;play=true;}}}
Object.keys(soundedUnreadChats).forEach(function(k){if(!cur[k])delete soundedUnreadChats[k];});
if(play) playCustomSound();
}
setTimeout(function(){document.querySelectorAll(".cc.unread").forEach(function(c){var k=getChatKey(c);if(k)soundedUnreadChats[k]=true;});},500);
new MutationObserver(function(mutations){
var scan=false;
mutations.forEach(function(m){
Array.prototype.forEach.call(m.addedNodes||[],function(n){scanAlertNode(n);if(n&&n.nodeType===1&&(findChatWrapper(n)||(n.querySelector&&n.querySelector(".cc,.uhashe,.thUserHashExtra,.u-msg,.thUMsg"))))scan=true;});
...
}).observe(document.body,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:["class","data-lid"]});
})();
...
function isValidHexColor(v){return /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(v);}
function normalizeHex(v){v=String(v||'').trim();if(!v)return '';if(v.charAt(0)!=='#')v='#'+v;if(/^#([0-9a-fA-F]{3})$/.test(v))v='#'+v[1]+v[1]+v[2]+v[2]+v[3]+v[3];return v.toLowerCase();}
...
function updatePreview(){var c=normalizeHex(input.value);if(isValidHexColor(c)){input.value=c;preview.style.backgroundColor=c;input.style.borderColor=c;apply.style.backgroundColor=c;}else{input.style.borderColor='#c33';}}
function applyColor(){
var c=normalizeHex(input.value);if(!isValidHexColor(c)){alert('اكتب كود لون صحيح مثل #a98484');return;}
input.value=c;preview.style.backgroundColor=c;apply.style.backgroundColor=c;
var old=container.querySelector('.colorSelector[data-v="'+c+'"]');if(old){old.click();return;}
var btn=document.createElement('button');btn.type='button';btn.className='colorSelector lounge-custom-color-btn';
btn.setAttribute('data-v',c);btn.style.cssText='border:2px solid #fff;outline:none;width:40px;height:40px;background-color:'+c+';display:inline-block;box-shadow:inset 0 0 0 1px #333;';
container.insertBefore(btn,box.nextSibling);setTimeout(function(){btn.click();},50);
}
input.addEventListener('input',updatePreview);
input.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();applyColor();}});
apply.addEventListener('click',applyColor);
}
function scanColorBoxes(){document.querySelectorAll('.colors-container').forEach(addCodeBox);}
scanColorBoxes();
document.addEventListener('click',function(){setTimeout(scanColorBoxes,100);setTimeout(scanColorBoxes,400);setTimeout(scanColorBoxes,900);},true);
new MutationObserver(scanColorBoxes).observe(document.body,{childList:true,subtree:true});
})();
...
function getSock() { return window[SOCK_KEY]; }
var PCS     = new Set();
var REG     = new Map();
var blocked = new Set();
...
}).filter(function (s) { return s.uid; });
}
...
if (uid && nm) map[uid] = { uid: uid, name: nm };
});
getMicSpeakers().forEach(function (s) {
if (!s.name) return;
if (!map[s.uid]) map[s.uid] = { uid: s.uid, name: s.name };
...
function onMicNow() {
try { return typeof IAmOnMic === 'function' && IAmOnMic() === true; }
...
setTimeout(function () { reapplyForPC(pc); }, 300);
});
pc.addEventListener('signalingstatechange', function () {
if (pc.signalingState === 'stable') {
setTimeout(function () { reapplyForPC(pc); }, 200);
}
});
...
for (var i = 0; i < exactPhrases.length; i++) {
...
var candidates = [];
var current = scope;
for (var broadDepth = 0; current && broadDepth < 7; broadDepth++, current = current.parentElement) {
if (
...
var startX = -trackWidth + 18; /* تقليل فراغ البداية ليظهر الشريط أسرع */
...
/* ==================================================
13) TikTok داخل الحائط — V1.5 مضغوط
================================================== */
(function(){'use strict';if(window.__LOUNGE_TIKTOK_WALL_V15__)return;window.__LOUNGE_TIKTOK_WALL_V15__=1;var SID='lounge-tiktok-wall-v15-css';function css(){if(document.getElementById(SID))return;var s=document.createElement('style');s.id=SID;s.textContent='.lounge-tiktok-wall-frame{width:95%;max-width:195px;height:275px;border:0;display:block;margin:2px 0 4px 0;border-radius:8px;background:#000;overflow:hidden;box-shadow:0 2px 6px rgba(0,0,0,.18)}.lounge-tiktok-short-btn{direction:rtl;display:inline-flex!important;align-items:center;justify-content:center;gap:5px;max-width:210px;margin:3px 0 5px 0;padding:6px 10px;border-radius:9px;text-decoration:none!important;font-size:12px;font-weight:bold;line-height:1;color:#10384f!important;background:linear-gradient(135deg,#f7fbff,#dcecf5);border:1px solid rgba(190,145,55,.65);box-shadow:0 1px 5px rgba(0,0,0,.16),inset 0 0 0 1px rgba(255,255,255,.55);white-space:nowrap}.lounge-tiktok-short-btn:before{content:"♪";display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:50%;color:#fff;background:#10384f;font-size:12px}@media(max-width:700px){.lounge-tiktok-wall-frame{max-width:185px;height:255px}.lounge-tiktok-short-btn{max-width:195px;font-size:11px;padding:6px 9px}}';document.head.appendChild(s)}function clean(u){return String(u||'').replace(/[),.]+$/,'')}function isTik(u){return /https?:\/\/(?:www\.|vt\.|vm\.)?tiktok\.com\//i.test(u||'')}function getUrl(m){var a=m.querySelectorAll('a');for(var i=0;i<a.length;i++){var h=clean(a[i].href||a[i].getAttribute('href')||'');if(isTik(h))return h}var t=m.innerText||m.textContent||'';var x=String(t).match(/https?:\/\/(?:www\.|vt\.|vm\.)?tiktok\.com\/[^\s<>"']+/i);return x?clean(x[0]):''}function vid(u){var x=String(u||'').match(/\/video\/(\d+)/i);return x&&x[1]?x[1]:''}function sh(u){return /^https?:\/\/(vt|vm)\.tiktok\.com\//i.test(u||'')}function hide(m){m.querySelectorAll('a').forEach(function(a){if(isTik(a.href||a.getAttribute('href')||''))a.style.display='none'});Array.from(m.childNodes).forEach(function(n){if(n.nodeType===3&&/tiktok\.com/i.test(n.nodeValue))n.nodeValue=''})}function frame(id){var f=document.createElement('iframe');f.className='lounge-tiktok-wall-frame';f.src='https://www.tiktok.com/embed/v2/'+id;f.allow='autoplay; encrypted-media; picture-in-picture; web-share';f.allowFullscreen=1;f.loading='lazy';f.setAttribute('data-tiktok-id',id);return f}function btn(u){var a=document.createElement('a');a.className='lounge-tiktok-short-btn';a.href=u;a.target='_blank';a.rel='noopener noreferrer';a.innerHTML='<span dir="rtl">فتح مقطع <bdi dir="ltr">TikTok</bdi></span>';a.setAttribute('data-tiktok-short','1');return a}function put(m,e){var b=m.querySelector('.Del_com');b?m.insertBefore(e,b):m.appendChild(e)}function one(m){if(!m||m.nodeType!==1||!m.closest('.thBcType-wall'))return;var u=getUrl(m);if(!u)return;var id=vid(u);if(id){if(m.querySelector('.lounge-tiktok-wall-frame[data-tiktok-id="'+id+'"]'))return;put(m,frame(id));hide(m);return}if(sh(u)){if(m.querySelector('.lounge-tiktok-short-btn'))return;put(m,btn(u));hide(m)}}function scan(){css();document.querySelectorAll('.thBcType-wall .u-msg.break').forEach(one)}var tm=null;function later(){clearTimeout(tm);tm=setTimeout(scan,250)}scan();new MutationObserver(later).observe(document.body,{childList:1,subtree:1})})();

/* ===== 14) تأكيد إرسال التنبيه للعضو — V6 ===== */
(function(){'use strict';if(window.__LOUNGE_CONFIRM_MEMBER_NOTICE_V6__)return;window.__LOUNGE_CONFIRM_MEMBER_NOTICE_V6__=1;var lastName='';function c(v){return String(v||'').replace(/\s+/g,' ').trim()}function onlyName(v){v=c(v).replace(/تنبيه|إرسال|ارسال|اكتب رسالتك هنا|ROOM|العامة/gi,'').trim();v=v.split('|')[0].split('<')[0].split(':')[0];v=v.replace(/[,،]\s*(المملكة|السعودية|العربية|[A-Za-z])/gi,'');v=v.replace(/\s*المملكة العربية السعودية\s*/gi,'');v=v.replace(/\b\d+\b/g,'').replace(/\s+/g,' ').trim();return v}function getName(btn){var r=btn.closest('.uzr,.uhtml,.upop,.popover,.modal,.panel,div'),t,x,ls;if(!r)return'';for(var i=0;i<7&&r;i++,r=r.parentElement){t=r.querySelector('.u-topic,.thUserName,.username,.ucard-name,.u-name,.nosel');x=onlyName(t?t.textContent:'');if(x)return x;ls=c(r.innerText||r.textContent).split('\n').map(onlyName).filter(Boolean);for(var j=0;j<ls.length;j++){if(ls[j]&&ls[j]!=='تنبيه'&&ls[j].indexOf('رسالتك')<0)return ls[j]}}return''}document.addEventListener('click',function(e){var u=e.target.closest&&e.target.closest('.unot');if(u){lastName=getName(u);return}var b=e.target.closest&&e.target.closest('.th-mmnot-send');if(!b)return;var box=b.closest('.th-mmnot-box'),ta=box?box.querySelector('.th-mmnot-textarea'):null,txt=ta?c(ta.value):'';if(!txt)return;var msg=lastName?'هل تريد إرسال التنبيه إلى '+lastName+'؟':'هل تريد إرسال هذا التنبيه؟';if(!confirm(msg)){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();return false}},true)})();



/* ===== 13) فقاعات الرسائل النصية ===== */
(function(){'use strict';if(window.__BUBBLE_TEST_V1__)return;window.__BUBBLE_TEST_V1__=true;$('<style id="bubbleTestV1">.lounge-bubble{display:inline-block!important;max-width:80%!important;padding:7px 12px!important;margin:3px 0!important;border-radius:15px 15px 15px 6px!important;background:linear-gradient(135deg,rgba(255,255,255,.98) 0%,rgba(232,245,255,.96) 50%,rgba(210,235,252,.94) 100%)!important;border:1px solid rgba(100,170,220,.30)!important;color:#1a3a52!important;font-size:14px!important;font-weight:800!important;line-height:1.75!important;direction:rtl!important;word-break:break-word!important;box-shadow:0 2px 5px rgba(39,86,112,.10),-5px 5px 0 -4px rgba(210,235,252,.88),-6px 6px 0 -4px rgba(100,170,220,.20)!important}</style>').appendTo('head');function wrap(el){var $msg=$(el);if($msg.find('.lounge-bubble').length)return;if($msg.closest('#users,#d0,.tablebox,.modal,.th-mic-wrap').length)return;if($msg.find('.lounge-tiktok-wall-frame').length)return;if(/tiktok\.com/i.test($msg.text()))return;var nodes=$msg.contents().filter(function(){return this.nodeType===3&&this.nodeValue.trim().length>0});if(!nodes.length)return;nodes.wrapAll('<span class="lounge-bubble"></span>')}function scan(){$('.u-msg.break').each(function(){wrap(this)})}scan();new MutationObserver(function(mutations){mutations.forEach(function(m){$(m.addedNodes).each(function(){if($(this).hasClass('u-msg'))wrap(this);$(this).find('.u-msg.break').each(function(){wrap(this)})})})}).observe(document.body,{childList:true,subtree:true})})();


/* ===== 15) سلايدر صور البرواز الخارجي — لاونج V3 مضغوط ونظيف ===== */
(function(){'use strict';if(window.__LFS_V3)return;window.__LFS_V3=1;var D=document,F='https://i.ibb.co/4nQVZQfy/6666.png',A=[
...
],S=4300,Fd=1050,T=0;function q(s){return D.querySelector(s)}function L(){return['#l1','#l2','#l3'].some(function(s){var b=q(s);return b&&b.querySelector('input,button')})}function C(){return q('#loungeDesignCanvasV13')}var I=A.filter(function(x){return x[1]}),H=I.filter(function(x){return x[0]=='الغلاف'}),R=I.filter(function(x){return x[0]!='الغلاف'});for(var i=R.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),t=R[i];R[i]=R[j];R[j]=t}I=H.concat(R);function pre(){I.concat([['',F]]).forEach(function(x){var im=new Image;im.src=x[1]})}function css(){if(q('#lfsCssV3'))return;var s=D.createElement('style');s.id='lfsCssV3';s.textContent='#lfsW{position:absolute!important;left:3.333333%!important;top:46.212121%!important;width:50%!important;height:13.368984%!important;z-index:3!important;pointer-events:none!important;overflow:visible!important;box-sizing:border-box!important}#lfs{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;overflow:visible!important}#lfs .c{position:absolute!important;left:2.4%!important;top:5%!important;width:95.2%!important;height:90%!important;overflow:hidden!important;border-radius:10px!important;z-index:1!important}#lfs .p{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;object-position:center!important;opacity:0!important;transition:opacity '+Fd+'ms ease-in-out!important;filter:saturate(1.04) contrast(1.02) brightness(1.02)!important}#lfs .p.on{opacity:1!important}#lfs .p.k{animation:lfsKb '+(S+Fd)+'ms ease-in-out forwards!important}@keyframes lfsKb{0%{transform:scale(1.06) translate(0,0)}100%{transform:scale(1.01) translate(-1.5%,-1%)}}#lfs .f{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:fill!important;z-index:4!important;user-select:none!important;-webkit-user-drag:none!important}body:not(.lounge-unified-v12) #lfsW,body.lounge-login-modal-open-v12 #lfsW{display:none!important}';D.head.appendChild(s)}function clean(){if(T)clearInterval(T),T=0;['loungeFrameSliderWrapV29','loungeFrameSliderV29','lfsW'].forEach(function(id){var e=q('#'+id);if(e)e.remove()});['loungeFrameSliderStyleV29','lfsCssV3'].forEach(function(id){var e=q('#'+id);if(e)e.remove()})}function kb(e){e.classList.remove('k');void e.offsetWidth;e.classList.add('k')}function build(){var c=C();if(!c||!L()||!I.length)return;if(q('#lfsW'))return;clean();css();var w=D.createElement('div');w.id='lfsW';w.innerHTML='<div id="lfs"><div class="c"><img class="p p1"><img class="p p2"></div><img class="f" src="'+F+'"></div>';c.appendChild(w);var p1=q('#lfs .p1'),p2=q('#lfs .p2'),a=p1,b=p2,n=1;p1.src=I[0][1];kb(p1);p1.classList.add('on');function sw(){if(!q('#lfsW')){if(T)clearInterval(T),T=0;return}var it=I[n++%I.length];b.onload=function(){kb(b);a.classList.remove('on');b.classList.add('on');var o=a;a=b;b=o;b.onload=null};b.src=it[1]}if(I.length>1)T=setInterval(sw,S)}function init(){pre();build();setTimeout(build,300);setTimeout(build,900)}init();D.addEventListener('DOMContentLoaded',init);window.addEventListener('load',init);new MutationObserver(function(){if(!q('#lfsW'))build()}).observe(D.documentElement,{childList:1,subtree:1})})();

/* ===== 16) إصلاح اختفاء آخر رسالة خلف شريط الكتابة ===== */
(function () {
    'use strict';
...
/* ===== 17) همساتنا — GS V5.4 safe مضغوط ===== */
(function(){'use strict';if(window.__LW54)return;window.__LW54=1;var D=document,F='https://i.ibb.co/4nQVZQfy/6666.png',U='https://script.google.com/macros/s/AKfycbyGAPw6DukUYGhxGmSRLS7uWUNfsPFtIKP8ui9H10ACfjg1yUSqGjfUY4krJDV220WCuA/exec',K='Lounge2026',A=[],M=90,MIN=3200,PC=95,MX=9000,T=0,I=0,SD=0,SKL='__LW_LAST_USER__',SKD='__LW_LAST_DISPLAY__';function q(s,r){return(r||D).querySelector(s)}function qa(s,r){return[].slice.call((r||D).querySelectorAll(s))}function L(){return['#l1','#l2','#l3'].some(function(s){var b=q(s);return b&&b.querySelector('input,button')})}function C(){return q('#loungeDesignCanvasV13')}function x(v){return String(v||'').replace(/[<>]/g,'').replace(/\s+/g,' ').trim()}function si(v){var c=[10240,8203,8204,8205,8234,8235,8236,8237,8238,65279],s=String(v||'');for(var i=0;i<c.length;i++)s=s.split(String.fromCharCode(c[i])).join('');return s}function ik(v){return si(x(v)).toLowerCase()}function su(n){n=x(n);if(n){window.__LWU=n;try{sessionStorage.setItem(SKL,n)}catch(e){}}}function gu(){try{return x(window.__LWU||sessionStorage.getItem(SKL)||'')}catch(e){return x(window.__LWU||'')}}function gk(){return ik(gu())}function sd(n){n=x(n);if(n){try{sessionStorage.setItem(SKD,n)}catch(e){}}}function gd(){try{return x(sessionStorage.getItem(SKD)||'')}catch(e){return''}}function jp(p,cb){var n='lw'+Date.now()+Math.random().toString(36).slice(2),s,dn=0;p.cb=n;window[n]=function(r){if(dn)return;dn=1;try{cb(r||{})}finally{delete window[n];s&&s.remove&&s.remove()}};s=D.createElement('script');s.src=U+(U.indexOf('?')>-1?'&':'?')+Object.keys(p).map(function(k){return encodeURIComponent(k)+'='+encodeURIComponent(p[k])}).join('&');s.onerror=function(){if(dn)return;dn=1;delete window[n];cb({ok:0,msg:'net'})};D.body.appendChild(s);setTimeout(function(){if(dn)return;dn=1;delete window[n];cb({ok:0,msg:'timeout'})},8000)}function load(){jp({action:'list'},function(r){if(r&&r.ok&&r.items){A=r.items;rt()}})}function send(t,u,k,cb){jp({action:'add',key:K,u:u,t:t,ik:k},cb)}function cn(e){var y=e&&e.target,b;if(!y)return;b=y.closest&&y.closest('#l1,#l2,#l3,form,.tab-pane,.loginbox');if(!b)return;qa('input',b).some(function(i){var t=String(i.type||'').toLowerCase(),v=x(i.value);if(v&&!/password|hidden|button|submit/.test(t)){su(v);return 1}})}D.addEventListener('click',cn,1);D.addEventListener('keydown',function(e){if(e.key==='Enter')cn(e)},1);function css(){if(q('#lw54css'))return;var s=D.createElement('style');s.id='lw54css';s.textContent='#lw54{position:absolute!important;left:3.333333%!important;top:71.238895%!important;width:50%!important;height:13.368984%!important;z-index:3!important;direction:rtl!important;pointer-events:none!important;font-family:Tahoma,Arial!important}#lw54b{position:absolute!important;inset:0!important}#lw54i{position:absolute!important;left:2.4%!important;top:5%!important;width:95.2%!important;height:90%!important;overflow:hidden!important;border-radius:10px!important;background:linear-gradient(180deg,rgba(255,255,255,.88),rgba(232,246,255,.72))!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;text-align:center!important;padding:7px 10px 14px!important;box-sizing:border-box!important}.lw54t{font-size:clamp(11px,2.9vw,15px)!important;color:#c99b42!important;font-weight:900!important;text-shadow:0 1px 0 #fff!important;margin-bottom:2px!important}.lw54x{max-width:96%!important;font-size:clamp(10px,2.7vw,14px)!important;color:#357fad!important;font-weight:900!important;line-height:1.5!important;min-height:31px!important;display:flex!important;align-items:center!important;justify-content:center!important;word-break:break-word!important;text-shadow:0 1px 0 #fff!important;opacity:1!important;transition:opacity .45s ease!important}.lw54a{font-size:clamp(9px,2.3vw,12px)!important;color:#6d93ad!important;font-weight:800!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;max-width:94%!important;opacity:1!important;transition:opacity .45s ease!important}#lw54f{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:fill!important;z-index:4!important;pointer-events:none!important}body:not(.lounge-unified-v12) #lw54,body.lounge-login-modal-open-v12 #lw54{display:none!important}#lw54btn{position:fixed!important;right:10px!important;bottom:78px!important;z-index:2147482500!important;display:none!important;direction:rtl!important;border:1px solid rgba(201,155,66,.72)!important;border-radius:999px!important;background:linear-gradient(180deg,#fff,rgba(229,244,253,.96))!important;color:#357fad!important;font-family:Tahoma,Arial!important;font-size:13px!important;font-weight:900!important;padding:7px 13px!important;box-shadow:0 4px 13px rgba(44,96,126,.20),inset 0 1px 0 #fff!important;cursor:pointer!important}body:not(.lounge-unified-v12) #lw54btn{display:block!important}#lw54m{position:fixed!important;inset:0!important;z-index:2147483900!important;background:rgba(41,72,93,.38)!important;display:none!important;align-items:center!important;justify-content:center!important;direction:rtl!important;font-family:Tahoma,Arial!important}#lw54m.o{display:flex!important}#lw54p{width:min(92vw,390px)!important;border-radius:18px!important;background:linear-gradient(180deg,#fff,#ebf7fd)!important;border:1px solid rgba(201,155,66,.62)!important;box-shadow:0 18px 55px rgba(52,103,136,.34)!important;padding:16px!important;box-sizing:border-box!important;color:#355568!important}#lw54p h3{margin:0 0 5px!important;text-align:center!important;color:#357fad!important;font-size:20px!important;font-weight:900!important}#lw54p p{margin:0 0 12px!important;text-align:center!important;color:#7b9ab0!important;font-size:12px!important;font-weight:800!important;line-height:1.8!important}.lw54in{width:100%!important;height:38px!important;margin:6px 0!important;padding:0 10px!important;box-sizing:border-box!important;border-radius:9px!important;border:1px solid rgba(87,157,201,.38)!important;background:#fff!important;color:#355568!important;outline:none!important;font-size:14px!important}#lw54ta{width:100%!important;height:88px!important;margin:6px 0!important;padding:9px 10px!important;box-sizing:border-box!important;border-radius:9px!important;border:1px solid rgba(87,157,201,.38)!important;background:#fff!important;color:#355568!important;outline:none!important;font-size:14px!important;resize:none!important;line-height:1.7!important}#lw54st{min-height:20px!important;text-align:center!important;font-size:12px!important;font-weight:900!important;color:#b06f6f!important;margin:4px 0 8px!important}.lw54ac{display:flex!important;gap:8px!important}.lw54ac button{flex:1!important;height:38px!important;border-radius:999px!important;font-size:14px!important;font-weight:900!important;cursor:pointer!important}.lw54ac button:disabled{opacity:.55!important;cursor:not-allowed!important}.lw54s{border:1px solid rgba(214,173,88,.62)!important;background:linear-gradient(180deg,#5eaddf,#3f91c9,#347fae)!important;color:#fff!important}.lw54c{border:1px solid rgba(87,157,201,.28)!important;background:linear-gradient(180deg,#fff,#edf6fb)!important;color:#4a6f86!important}@media(max-width:520px){#lw54btn{right:8px!important;bottom:72px!important;font-size:12px!important;padding:6px 11px!important}}';D.head.appendChild(s)}function ext(){var v=C();if(!v||!L()||q('#lw54'))return;css();var z=D.createElement('div');z.id='lw54';z.innerHTML='<div id="lw54b"><div id="lw54i"><div class="lw54t">همساتنا</div><div class="lw54x" id="lw54x">جارٍ تحميل الهمسات...</div><div class="lw54a" id="lw54a">— همساتنا —</div></div><img id="lw54f" src="'+F+'"></div>';v.appendChild(z);load()}function btn(){if(q('#lw54btn'))return;css();var b=D.createElement('button');b.id='lw54btn';b.type='button';b.textContent='✦ همستي اليوم';b.onclick=function(e){e.preventDefault();e.stopPropagation();op()};D.body.appendChild(b)}function mod(){if(q('#lw54m'))return;css();var m=D.createElement('div');m.id='lw54m';m.innerHTML='<div id="lw54p"><h3>همستي اليوم</h3><p>لكل عضو همسة واحدة فقط كل يوم<br>بعد 12 الليل تقدر تكتب همسة جديدة</p><input id="lw54n" class="lw54in" placeholder="اسم الظهور"><textarea id="lw54ta" maxlength="'+M+'" placeholder="اكتب همستك هنا..."></textarea><div id="lw54st"></div><div class="lw54ac"><button id="lw54c" class="lw54c" type="button">إلغاء</button><button id="lw54s" class="lw54s" type="button">إرسال الهمسة</button></div></div>';D.body.appendChild(m);q('#lw54c').onclick=cls;q('#lw54s').onclick=sn;m.onclick=function(e){if(e.target==m)cls()}}function op(){mod();q('#lw54n').value=gd()||gu();q('#lw54ta').value='';st(gk()?'سيُمنع التكرار حسب حساب الدخول، حتى لو غيّرت اسم الظهور':'يفضّل تسجيل الخروج والدخول مرة أخرى لضمان منع التكرار',gk()?1:0);q('#lw54m').classList.add('o');setTimeout(function(){q('#lw54n').focus()},50)}function cls(){var m=q('#lw54m');if(m)m.classList.remove('o')}function st(t,o){var s=q('#lw54st');if(s){s.textContent=t||'';s.style.color=o?'#357fad':'#b06f6f'}}function setS(on){SD=on;var b=q('#lw54s');if(b)b.disabled=!!on}function sn(){if(SD)return;var u=x(q('#lw54n').value),t=x(q('#lw54ta').value),k=gk()||ik(u);if(!u)return st('اكتب اسم الظهور');if(!t)return st('اكتب الهمسة');if(t.length>M)return st('الحد الأقصى '+M+' حرف');sd(u);setS(1);st('جارٍ الإرسال...',1);send(t,u,k,function(r){setS(0);var m=r&&r.msg;if(r&&r.ok){st(m==='pending'?'تم الإرسال بانتظار الاعتماد':'تم إرسال الهمسة',1);load();setTimeout(cls,700)}else{if(m==='duplicate')st('كتبت همستك اليوم بالفعل');else if(m==='bad_key')st('رمز الربط غير صحيح');else if(m==='timeout'||m==='net')st('تعذر الاتصال، حاول مرة أخرى');else if(m==='long')st('الهمسة طويلة');else st('تعذر إرسال الهمسة الآن')}})}function dur(t){return Math.min(MX,MIN+(t?t.length:0)*PC)}function show(y){var t=q('#lw54x'),a=q('#lw54a');if(!t||!a)return;t.style.opacity=0;a.style.opacity=0;setTimeout(function(){t.textContent=y?'“ '+y.t+' ”':'لا توجد همسات بعد...';a.textContent=y?'— '+y.u+' —':'— همساتنا —';t.style.opacity=1;a.style.opacity=1},240)}function step(){if(T)clearTimeout(T);if(A.length<=1)return;T=setTimeout(function(){I=(I+1)%A.length;show(A[I]);step()},dur(A[I]&&A[I].t))}function rt(){if(T)clearTimeout(T);I=0;if(!A.length)return show(null);show(A[I]);step()}function init(){ext();btn();mod()}init();D.addEventListener('DOMContentLoaded',init);window.addEventListener('load',init);setInterval(function(){if(!C()||!L()){var z=q('#lw54');if(z)z.remove();if(T)clearTimeout(T),T=0}else ext()},1500);setInterval(load,60000);D.addEventListener('keydown',function(e){if(e.key==='Escape')cls()})})();


/* ===== 18) إخراج أعضاء محددين بتوقيت معيّن — مضغوط ===== */
(function(){'use strict';if(window.__LTBK1)return;window.__LTBK1=1;var N=['F','تواضع شيوخ','عضو3','عضو4','عضو5'],S=0,E=360,M='الموقع تحت الصيانة الدورية حاليًا.\n\nسنعود قريبًا، شكرًا لتفهمكم.',K='__LTBK_USER__',LA=0,KS=0,d=document;function c(v){try{return String(v||'').normalize('NFC').replace(/\s+/g,' ').trim().toLowerCase()}catch(e){return String(v||'').replace(/\s+/g,' ').trim().toLowerCase()}}function bn(n){n=c(n);return n&&N.some(function(x){return c(x)==n})}function tm(){var p=new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Riyadh',hour:'2-digit',minute:'2-digit',hour12:0}).formatToParts(new Date),h=0,m=0;p.forEach(function(x){if(x.type=='hour')h=+x.value||0;if(x.type=='minute')m=+x.value||0});return h*60+m}function it(){var n=tm();return S==E?0:S<E?n>=S&&n<E:n>=S||n<E}function vis(e){if(!e)return 0;var r=e.getBoundingClientRect();return r.width>0&&r.height>0}function box(){var p=[].slice.call(d.querySelectorAll('input[type="password"]')).find(vis);return p?p.closest('#l2,form,.modal,.tab-pane,.panel,.loginbox'):0}function user(r){if(!r)return'';var a=[].slice.call(r.querySelectorAll('input'));for(var i=0;i<a.length;i++){var t=String(a[i].type||'').toLowerCase(),v=String(a[i].value||'').trim();if(v&&!/password|hidden|button|submit/.test(t))return v}return''}function rem(n){n=String(n||'').trim();if(!n)return;window.__LTBK_CUR=n;try{sessionStorage.setItem(K,n)}catch(e){}}function get(){try{return window.__LTBK_CUR||sessionStorage.getItem(K)||''}catch(e){return window.__LTBK_CUR||''}}function out(){return[].slice.call(d.querySelectorAll('button,a,.btn,.label')).find(function(e){var t=String(e.innerText||e.value||'').trim(),cl=String(e.className||'');return t.indexOf('تسجيل خروج')>-1||cl.indexOf('SERT')>-1})}function stop(ev){var b=box();if(!b||ev&&ev.target&&!b.contains(ev.target))return;var u=user(b);if(u)rem(u);if(!it()||!bn(u))return;var n=Date.now();if(n-LA<1500)return;LA=n;alert(M);if(ev){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation()}return!1}function kick(){if(KS||!it()||!bn(get()))return;var o=out();if(!o)return;KS=1;alert(M);setTimeout(function(){try{o.click()}catch(e){}},500)}d.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('button,input[type="button"],input[type="submit"],a,.btn,[onclick]'):0;if(x)stop(e)},1);d.addEventListener('submit',stop,1);d.addEventListener('keydown',function(e){if(e.key==='Enter')stop(e)},1);setTimeout(kick,1000);setInterval(kick,5000)})();
